...
static void **array_to_sort;

static void Swap(uint32_t first_index, uint32_t second_index);

static void QuickSortIntHelper(uint32_t lo, uint32_t hi,
  int32_t (*comparator)(void *, void *));

static uint32_t FindPivot(uint32_t lo, uint32_t hi,
  int32_t (*comparator)(void *, void *));

void QuickSortInt(void *arr[], uint32_t size,
  int32_t (*comparator)(void *, void *)) {
  array_to_sort = arr;
...
static void QuickSortIntHelper(uint32_t lo, uint32_t hi,
  ...
  void *pivot_value = array_to_sort[pivot];
 ...
 */
static void Swap(uint32_t first_index, uint32_t second_index) {
  void *temp = array_to_sort[first_index];
...
static uint32_t FindPivot(uint32_t lo, uint32_t hi,
  ...
  if (comparator(array_to_sort[lo], array_to_sort[hi - 1]) *
    ...
    // array_to_sort[lo] is in between array_to_sort[hi - 1] and
    // array_to_sort[mid]
    return lo;
  } else if (comparator(array_to_sort[mid], array_to_sort[hi - 1]) *
             comparator(array_to_sort[mid], array_to_sort[lo]) < 0) {
    // array_to_sort[mid] is in between array_to_sort[hi - 1] and
    // array_to_sort[lo]
    return mid;
  } else {
    // array_to_sort[hi - 1] is in between array_to_sort[lo] and
    // array_to_sort[mid]
    return hi - 1;
  }
}
